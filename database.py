...
def get_request_job() -> int:
    session = Session()
    try:
        users = session.query(Job).filter(Job.status == JobType.READY).order_by(asc(Job.id)).limit(1).all()
...
def list_news_by_job_id(job_id: int) -> Optional[list]:
    session = Session()
    try:
        corpus_list = session.query(Corpus).filter(Corpus.job_id == job_id).all()
        if corpus_list is None or len(corpus_list) == 0:
            return None
        news_list = []
        for item in corpus_list:
            news_list.append({"title": item.title, "content": item.content, "uid": item.uid})
        return news_list
    except SQLAlchemyError as e:
        session.rollback()
...
def add_cluster(job_id: int, cluster_id: int, keywords: str) -> bool:
    session = Session()
    try:
        cluster = Cluster(job_id=job_id, cluster_id=cluster_id, keywords=keywords)
        session.add(cluster)
        session.commit()
        return True
    except SQLAlchemyError as e:
